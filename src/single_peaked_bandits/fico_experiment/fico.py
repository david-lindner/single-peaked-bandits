"""
Reading FICO Data
Source: https://github.com/lydiatliu/delayedimpact
"""
...
def read_totals(data_dir=DATA_DIR):
    ...
    frame = cleanup_frame(pd.read_csv(data_dir + FILES["overview"], index_col=0))
...
def parse_data(data_dir=DATA_DIR, filenames=None):
    ...
    cdfs = cleanup_frame(pd.read_csv(data_dir + filenames[0], index_col=0))
    performance = 100 - cleanup_frame(pd.read_csv(data_dir + filenames[1], index_col=0))
